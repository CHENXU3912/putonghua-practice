...
import { scoreSpeech } from '@/lib/scorer';
...
export default function SpeakingPage() {
  ...
  const handleSubmit = useCallback(() => {
    const s = scoreSpeech(speech, audioDuration, selfRating as 1|2|3|4|5, outlineCoverage as 1|2|3|4|5);
    setScoreResult(s);
    setShowResult(true);
    ...
    doCheckin();
  }, [speech, audioDuration, selfRating, outlineCoverage, blob]);
    ...
    return (
      <div className="px-4 py-6 space-y-5">
        ...
        <div className="flex gap-3">
          <button onClick={() => { setShowResult(false); setScoreResult(null); setBlob(null); setBlobUrl(null); setAudioDuration(0); }} className="flex-1 py-3 bg-green-500 text-white rounded-xl font-medium active:bg-green-600 transition">重新练习</button>
  ...
  return (
    <div className="px-4 py-6">
      ...
      {/* 录音区 */}
      <div className="mb-6">
        <Recorder onResult={handleRecord} maxDuration={180} />
        {audioDuration > 0 && (
              ...
              <div className="flex justify-center gap-2">
                {[1, 2, 3, 4, 5].map(s => (
                  <button key={s} onClick={() => setSelfRating(s)}
                    className={`text-3xl transition ${s <= selfRating ? 'text-yellow-400' : 'text-gray-200'}`}>★</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex justify-center gap-2">
                {[1, 2, 3, 4, 5].map(s => (
                  <button key={s} onClick={() => setOutlineCoverage(s)}
                    className={`text-3xl transition ${s <= outlineCoverage ? 'text-blue-400' : 'text-gray-200'}`}>◆</button>
                ))}
              </div>
            </div>
          </div>
