...
export default function SyllablePage() {
  ...
  const handleNext = useCallback(async () => {
    ...
    resultsRef.current[currentIdx] = {
      itemId: item!.id,
      audioDuration: durationsRef.current[currentIdx] || 1.5,
    ...
    if (currentIdx < items.length - 1) {
      ...
      s.wrongItems.forEach(w => {
        const idx = items.findIndex(it => it.char === w.content);
              ...
              return (
                <div key={it.id} className={`flex items-center gap-3 py-2 px-2 rounded-lg ${ok ? '' : 'bg-red-50'}`}>
                  ...
                  <div className="flex-1 flex flex-wrap gap-1">
                    {it.errorType.map((t, j) => (
                      <span key={j} className={`px-1.5 py-0.5 rounded text-2xs ${
                        !ok && (t.includes('平舌') || t.includes('翘舌'))
                          ? 'bg-purple-100 text-purple-700 font-medium' : 'bg-gray-100 text-gray-500'
                      }`}>{t}</span>
                    ))}
                  </div>
                  {!ok && (
                    <span className="text-xs text-red-500 whitespace-nowrap">
                      {it.errorType.includes('平舌音') || it.errorType.includes('翘舌音') ? '平翘舌混淆' :
                       it.errorType.includes('前鼻音') || it.errorType.includes('后鼻音') ? '前后鼻音混淆' :
                       it.errorType.includes('边音(l)') || it.errorType.includes('鼻音(n)') ? 'n/l混淆' :
                       it.errorType.includes('f/h混淆') ? 'f/h混淆' :
                       it.errorType.includes('送气音') || it.errorType.includes('不送气音') ? '送气/不送气混淆' :
                       it.errorType[0] || '发音需练习'}
                    </span>
                  )}
                  {!ok && wrongItemIds.has(i) && (
                    <button onClick={() => handleAddWrong(i)} className="ml-1 px-2 py-1 bg-orange-100 text-orange-600 rounded text-xs whitespace-nowrap active:bg-orange-200">
                      +错音本
                    </button>
                  )}
                  {!ok && !wrongItemIds.has(i) && (
                    <span className="text-xs text-green-500 ml-1">已添加</span>
                  )}
                </div>
              );
            })}
  ...
  const errorReason = item.errorType.includes('平舌音') || item.errorType.includes('翘舌音') ? '平翘舌混淆' :
    ...
    item.errorType.includes('f/h混淆') ? 'f/h混淆' :
    item.errorType.includes('送气音') || item.errorType.includes('不送气音') ? '送气/不送气混淆' :
